...
def generate_dot_variants(username):
    return [
        ''.join(part) for part in set(
            map(lambda x: x.strip('.'), [
                f"{username[:i]}.{username[i:]}" for i in range(1, len(username))
            ])
        ) if len(username) > 1
    ]
...
async def account_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Select an account type:", reply_markup=InlineKeyboardMarkup(keyboard))
